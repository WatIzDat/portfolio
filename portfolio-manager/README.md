...
* [`src/portfolio_manager/`](src/portfolio_manager/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/portfolio_manager/core.cljs): contains the SPA entry point, `init`
...
5. Clone this repo and open a terminal in the `portfolio-manager` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/portfolio_manager/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns portfolio-manager.example
  (:require [portfolio-manager.config :as config])
